...
namespace FFW_TT_BuffBlock
{
    internal class Logger
    {
        ...
        {
            BuffBlocks.logger.Trace($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffff")} | {(LogLevel)level} | {loggerID} | {message}");
        }

        private void LogException(byte level, Exception exception)
        {
            BuffBlocks.logger.Trace($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffff")} | {(LogLevel)level} | {loggerID} | {exception.ToString()}");
        }
